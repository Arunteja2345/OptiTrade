...
//   const [expiry] = useState('2025-10-14');
...
//       <table className="w-full table-auto border-separate border border-solid border-gray-400">
//           <thead>
...
export default function OptionChain() {
  ...
  const [loading, setLoading] = useState(false);
  const [index] = useState('NIFTY');
  const [expiry, setExpiry] = useState(''); // User input expiry

  const fetchOptionChain = async (selectedExpiry) => {
    const token = localStorage.getItem('token');
    if (!token) {
      setError('Session expired, please log in again');
      logout();
    ...
    try {
      const response = await axios.get(
        `http://localhost:3001/api/option-chain/${index}?expiry=${selectedExpiry}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      if (Array.isArray(response.data) && response.data.length > 0) {
        ...
        setError('No option chain data found for this expiry. Please select a correct date.');
        setOptionChain([]);
      }
    } catch (error) {
      if (error.response?.status === 401) {
  ...
  const handleFetchClick = () => {
    fetchOptionChain(expiry);
  };

  return (
    <div className="bg-white p-4 rounded-lg shadow-md">
      <h3 className="text-lg font-semibold mb-2">Option Chain - {index}</h3>

      <div className="mb-4">
        <label className="mr-2 font-medium">Select Expiry Date: </label>
        <input
          type="date"
          value={expiry}
