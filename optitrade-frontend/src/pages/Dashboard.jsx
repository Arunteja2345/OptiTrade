...
export default function Dashboard() {
  ...
  const fetchTrades = async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        setError('Failed to fetch trades: ' + (error.response?.data.error || error.message));
  ...
  const fetchSummary = async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        setError('Failed to fetch summary: ' + (error.response?.data.error || error.message));
  ...
  const handleUpdateLTPs = async () => {
    ...
    try {
      ...
      if (response.data?.trades && Array.isArray(response.data.trades)) {
        ...
        setError(
          response.data?.message || 'No trades updated or invalid response format'
        );
      ...
      if (!error.response) {
        ...
        setError(
          `Failed to update LTPs: ${error.response?.data?.error || error.message}`
        );
      }
    } finally {
      setIsUpdating(false);
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: 'Profit Points',
        data: trades.map((trade) => trade.profitPoints != null ? trade.profitPoints : 0),
